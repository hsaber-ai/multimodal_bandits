...
class MAB:
    def __init__(self,arms, structure = '', Theta = []):
...
def GaussianBandit(means):
    """define a Bernoulli MAB from a vector of means"""
    return MAB([arms.Gaussian(p) for p in means])

def ExponentialBandit(means):
    """define a Bernoulli MAB from a vector of means"""
    return MAB([arms.Exponential(p) for p in means])
